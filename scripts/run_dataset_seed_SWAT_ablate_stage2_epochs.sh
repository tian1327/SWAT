...
# folder="swat_ablate_stage2_epochs"
folder="swat_ablate_stage2_epochs_trainingseed"
...
echo "epoch,Dataset,Method,DataSource,Init,Shots,Seed,Retrieve,Stage1Acc,WSFT,Stage2Acc" > "$output_file"
                ...
                for init in "${cls_inits[@]}"; do                
                    for epoch in "${epochs[@]}"; do
                        for seed in "${seeds[@]}"; do
                            for train_seed in "${training_seeds[@]}"; do
                                for retrieval_split in "${retrieval_splits[@]}"; do
                                    ...
                                    if [ "$dataset" == "imagenet" ]; then
                                        model_path="output/swat_vitb32_T2T500/output_${dataset}/${dataset}_cutmix_fewshot+retrieved_REAL-Prompt_16shots_seed1_10eps/stage1_model_best-epoch_10_best.pth"
                                    else
                                    ...
                                    --cls_init "$init" --shots "$shots" --seed "$seed" --training_seed "$train_seed" \
                                    --epochs "$epoch" --bsz "$batch_size" \
                                    --log_mode "$log_mode" --retrieval_split "${retrieval_split}.txt" --model_cfg "$model_cfg" \
                                    --no_wsft --no_tau --skip_stage2 \
