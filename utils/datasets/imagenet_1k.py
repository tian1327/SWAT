...
# for ImageNet-Adv dataset use
adv_thousand_k_to_200 = {0: -1, 1: -1, 2: -1, 3: -1, 4: -1, 5: -1, 6: 0, 7: -1, 8: -1, 9: -1, 10: -1,
                            11: 1, 12: -1, 13: 2, 14: -1, 15: 3, 16: -1, 17: 4, 18: -1, 19: -1, 20: -1,
                            21: -1, 22: 5, 23: 6, 24: -1, 25: -1, 26: -1, 27: 7, 28: -1, 29: -1, 30: 8, 31: -1, 32: -1,
                            33: -1, 34: -1, 35: -1, 36: -1, 37: 9, 38: -1, 39: 10, 40: -1, 41: -1, 42: 11, 43: -1, 44: -1,
                            45: -1, 46: -1, 47: 12, 48: -1, 49: -1, 50: 13, 51: -1, 52: -1, 53: -1, 54: -1, 55: -1, 56: -1,
                            57: 14, 58: -1, 59: -1, 60: -1, 61: -1, 62: -1, 63: -1, 64: -1, 65: -1, 66: -1, 67: -1, 68: -1,
                            69: -1, 70: 15, 71: 16, 72: -1, 73: -1, 74: -1, 75: -1, 76: 17, 77: -1, 78: -1, 79: 18, 80: -1,
                            81: -1, 82: -1, 83: -1, 84: -1, 85: -1, 86: -1, 87: -1, 88: -1, 89: 19, 90: 20, 91: -1, 92: -1,
                            93: -1, 94: 21, 95: -1, 96: 22, 97: 23, 98: -1, 99: 24, 100: -1, 101: -1, 102: -1, 103: -1,
                            104: -1, 105: 25, 106: -1, 107: 26, 108: 27, 109: -1, 110: 28, 111: -1, 112: -1, 113: 29, 114: -1,
...
indices_in_1k_adv = [k for k in adv_thousand_k_to_200 if adv_thousand_k_to_200[k] != -1]

# for ImageNet-Ren dataset use
...
imagenet_r_mask = [wnid in imagenet_r_wnids for wnid in all_wnids] # True or false for each wnid
imagenet_r_indices = [i for i in range(1000) if imagenet_r_mask[i] is True]
ren_thousand_k_to_200 = {k: v for k, v in zip(imagenet_r_indices, range(200))}
indices_in_1k_ren = imagenet_r_indices
...
class ImageNet1KDataset(Dataset):
    def __init__(self, transform=None, dataset_root=".", caption_map=None):
        ...
        if len(self.fnames) == 0:
            raise ValueError('Check image suffic. No images found in the specified dataset_root: {}'.format(dataset_root))

        self.transform = transform
        self.caption_map = caption_map
        # load a json file to the label_index_map
        # self.label_index_map = json.load(open('analysis/imagenet_label_map.json', 'r'))
    ...
    def __getitem__(self, i):
        ...
        # print(f'fname: {self.fnames[i]}, folder_name: {folder_name}, img_id: {img_id}, label: {label}')
        # stop
...
class ImageNetSketchDataset(Dataset):
    def __init__(self, transform=None, dataset_root="."):
        self.dataset_root = pathlib.Path(dataset_root)
        self.fnames = list(self.dataset_root.glob("**/*.JPEG"))
        self.transform = transform
        # load a json file to the label_index_map
        self.label_index_map = json.load(open('data/imagenet/imagenet_label_map.json', 'r'))

    def __len__(self):
        return len(self.fnames)

    def __getitem__(self, i):
        img, label = Image.open(self.fnames[i]), self.label_index_map[self.fnames[i].parent.name]
        if self.transform is not None:
            img = self.transform(img)
...
class ImageNetAdvDataset(Dataset):
    def __init__(self, transform=None, dataset_root=".", caption_map=None):
        self.dataset_root = pathlib.Path(dataset_root)
        self.fnames = sorted(list(self.dataset_root.glob("**/*.jpg")))
        self.transform = transform
        # load a json file to the label_index_map
        self.label_index_map = json.load(open('data/imagenet/imagenet_label_map.json', 'r'))
        self.caption_map = caption_map
    ...
    def __getitem__(self, i):
        img = Image.open(self.fnames[i])
        folder_name = self.fnames[i].parent.name
        index_in_1k = self.label_index_map[folder_name]
        label = adv_thousand_k_to_200[index_in_1k]
        # print('fname: {}, folder_name: {}, index_in_1k: {}, label: {}'.format(self.fnames[i], folder_name, index_in_1k, label))
        # stop
...
class ImageNetRenDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        folder_name = self.fnames[i].parent.name
        index_in_1k = self.label_index_map[folder_name]
        label = ren_thousand_k_to_200[index_in_1k]
        # print('fname: {}, folder_name: {}, index_in_1k: {}, label: {}'.format(self.fnames[i], folder_name, index_in_1k, label))
        # stop
        if self.transform is not None:
            img = self.transform(img)
